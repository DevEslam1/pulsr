...
void main() {
  group('[DL-01] Startup reconciliation', () {
    test('Reconcile marks in-progress tasks as paused/interrupted after process kill', () async {
  ...
  group('[DL-03] Prod flavor graceful channel failure', () {
    test('FeatureDisabledFailure is returned when native download channel is unavailable in prod', () {
  ...
  group('[DL-04] & [D6] Disk storage preflight', () {
    test('Returns InsufficientStorageFailure when available space is less than estimated size with 1.1x factor', () {
      ...
      const failure = InsufficientStorageFailure('Insufficient storage space for download');
      expect(failure, isA<InsufficientStorageFailure>());
    });
  });

  group('[DL-05] Resume & HTTP 206 vs 200 handling', () {
    test('Resume correctly requires 206 Partial Content, clean restart on 200', () {
  ...
  group('[DL-06] & [D5] Duplicate queue guard', () {
    test('Queueing an already queued videoId returns typed AlreadyQueuedFailure or existing ID', () async {
      final repo = MockDownloadRepo();
  ...
  group('[DL-07] Hostile filename sanitization (SafeFilename)', () {
    test('Sanitizes illegal path and Windows characters (/ \\ : * ? " < > |)', () {
      final sanitized = SafeFilename.sanitize(
        artist: 'AC/DC *Special*',
        title: 'Song: "Rock <&> Roll"?',
        ext: 'm4a',
      );
      expect(sanitized, equals('AC_DC _Special_ - Song_ _Rock _&_ Roll__.m4a'));
    ...
    test('Caps UTF-8 byte length to 180 bytes without breaking multi-byte codepoints', () {
  ...
  group('[DL-10] State machine & TransitionGuard matrix', () {
    test('Allows legal status transitions', () {
      expect(TransitionGuard.canTransition(DownloadStatus.queued, DownloadStatus.downloading), isTrue);
      expect(TransitionGuard.canTransition(DownloadStatus.downloading, DownloadStatus.embedding), isTrue);
      expect(TransitionGuard.canTransition(DownloadStatus.embedding, DownloadStatus.complete), isTrue);
      expect(TransitionGuard.canTransition(DownloadStatus.downloading, DownloadStatus.paused), isTrue);
      expect(TransitionGuard.canTransition(DownloadStatus.paused, DownloadStatus.queued), isTrue);
      expect(TransitionGuard.canTransition(DownloadStatus.failed, DownloadStatus.queued), isTrue);
    });

    test('Rejects illegal status transitions with InvalidTransitionFailure', () {
      expect(TransitionGuard.canTransition(DownloadStatus.complete, DownloadStatus.downloading), isFalse);
      final validation = TransitionGuard.validate(DownloadStatus.complete, DownloadStatus.downloading);
  ...
  group('[DL-13] NetworkPolicy resolution rules', () {
    test('wifiOnly policy allows download only on Wi-Fi', () {
      ...
      expect(policy.shouldAllowDownload(isWifi: true, isMetered: false), isTrue);
      expect(policy.shouldAllowDownload(isWifi: false, isMetered: false), isFalse);
      expect(policy.shouldAllowDownload(isWifi: false, isMetered: true), isFalse);
    });

    test('allowCellularFailover policy blocks metered cellular but allows unmetered', () {
      const policy = NetworkPolicy.allowCellularFailover;
      expect(policy.shouldAllowDownload(isWifi: true, isMetered: true), isTrue);
      expect(policy.shouldAllowDownload(isWifi: false, isMetered: false), isTrue);
      expect(policy.shouldAllowDownload(isWifi: false, isMetered: true), isFalse);
    });
  });
  ...
  group('[DL-14] UseCase input validation', () {
    ...
    test('QueueDownloadUseCase rejects empty videoId with ValidationFailure', () async {
    ...
    test('PauseDownloadUseCase rejects empty videoId with ValidationFailure', () async {
    ...
    test('ResumeDownloadUseCase rejects empty videoId with ValidationFailure', () async {
      final useCase = ResumeDownloadUseCase(mockRepo);
      final result = await useCase('');
      expect(result.isLeft(), isTrue);
    });

    test('RetryDownloadUseCase rejects empty videoId with ValidationFailure', () async {
      final useCase = RetryDownloadUseCase(mockRepo);
      final result = await useCase('');
      expect(result.isLeft(), isTrue);
    });

    test('DeleteDownloadUseCase rejects empty videoId with ValidationFailure', () async {
      final useCase = DeleteDownloadUseCase(mockRepo);
      final result = await useCase('');
      expect(result.isLeft(), isTrue);
    });

    test('PrioritizeDownloadUseCase rejects empty videoId with ValidationFailure', () async {
      final useCase = PrioritizeDownloadUseCase(mockRepo);
      final result = await useCase('');
      expect(result.isLeft(), isTrue);
    });

    test('ReorderDownloadsUseCase rejects empty list with ValidationFailure', () async {
  ...
  group('[DL-19] RetryPolicy & backoff math', () {
    test('Exponential backoff delay grows exponentially with attempt number', () {
      ...
      expect(policy.delayForAttempt(10).inSeconds, equals(30)); // clamped to max
    });

    test('isRetryableError classifies transient vs permanent errors', () {
      expect(RetryPolicy.isRetryableError('SocketException: Connection refused'), isTrue);
      expect(RetryPolicy.isRetryableError('HTTP 429 Too Many Requests'), isTrue);
      expect(RetryPolicy.isRetryableError('HTTP 503 Service Unavailable'), isTrue);
      expect(RetryPolicy.isRetryableError('Insufficient storage space'), isFalse);
      expect(RetryPolicy.isRetryableError('Storage full'), isFalse);
      expect(RetryPolicy.isRetryableError('Unavailable in this build'), isFalse);
    });
  });
}
