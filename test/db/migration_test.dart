...
void main() {
    ...
    test('YouTube rows coexist with local rows and remote_id is unique',
      ...
      await db.into(db.songsTable).insert(
            SongsTableCompanion.insert(
              ...
              remoteArtworkUrl:
                  const Value('https://i.ytimg.com/vi/abc123/hq.jpg'),
            ),
          );
      ...
      for (final id in [201, 202]) {
        await db.into(db.songsTable).insert(
              SongsTableCompanion.insert(
                  id: Value(id), title: 'Local $id', path: '/music/$id.mp3'),
            );
      }
      final locals = await (db.select(db.songsTable)
    ...
    test('ExcludedFolders table is created and operational', () async {
      final insertedId = await db.into(db.excludedFoldersTable).insert(
            ExcludedFoldersTableCompanion.insert(
                folderPath: '/storage/emulated/0/WhatsApp'),
          );
  ...
  group('Upgrade path', () {
    ...
    test('v4 -> v7 backfills source, keeps data, and adds the new indexes',
        () async {
      ...
      expect(song.playCount, equals(9),
          reason: 'existing rows must survive addColumn');
      expect(song.source, equals(SongSource.local),
    ...
    test(
        'v2 -> v7 runs the whole ladder without indexing a column that does not exist yet',
        () async {
