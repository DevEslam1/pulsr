...
class TestPulsrAudioHandler extends BaseAudioHandler
  ...
  BehaviorSubject<MediaItem?> get mediaItem =>
      BehaviorSubject<MediaItem?>.seeded(null)..addStream(_mediaItemController.stream);
  ...
  BehaviorSubject<List<MediaItem>> get queue =>
      BehaviorSubject<List<MediaItem>>.seeded([])..addStream(_queueController.stream);

  final StreamController<PlaybackState> _playbackStateController =
      StreamController<PlaybackState>.broadcast();
  @override
  BehaviorSubject<PlaybackState> get playbackState =>
      BehaviorSubject<PlaybackState>.seeded(PlaybackState())..addStream(_playbackStateController.stream);
...
void main() {
  ...
  group('PlayerCubit', () {
    ...
    test('[E4] close() during playback: no throw, 0 active subscriptions', () async {
      final cubit = PlayerCubit(
        audioHandler: testAudioHandler,
        repository: mockRepository,
        toggleFavoriteUseCase: mockToggleFavorite,
    ...
    test('[E4] 1000 position ticks in 10s -> throttled emissions to seek-bar selector, 0 to metadata selector', () async {
      final cubit = PlayerCubit(
        audioHandler: testAudioHandler,
        repository: mockRepository,
        toggleFavoriteUseCase: mockToggleFavorite,
      ...
      final initialMeta = (cubit.state.currentSong?.id, cubit.state.isPlaying, cubit.state.queue.length);
      ...
      for (int i = 1; i <= 100; ++i) {
        testAudioHandler._positionController.add(Duration(milliseconds: i * 10));
      }
      ...
      expect(metadataEmissions, equals(0)); // 0 emissions to track metadata selector
    ...
    test('[E4] Track change mid-tick: no stale-position emission for the old track', () async {
      final cubit = PlayerCubit(
        audioHandler: testAudioHandler,
        repository: mockRepository,
        toggleFavoriteUseCase: mockToggleFavorite,
    ...
    test('[E4] Comprehensive audio effects & state methods coverage test', () async {
      final cubit = PlayerCubit(
        audioHandler: testAudioHandler,
        repository: mockRepository,
        toggleFavoriteUseCase: mockToggleFavorite,
      ...
      await cubit.setLookaheadLimiter(true, thresholdDb: -0.3, releaseMs: 60.0);
      ...
      // Sleep timers
      cubit.startSleepTimer(30);
      expect(cubit.state.sleepTimerRemaining, equals(const Duration(minutes: 30)));
      cubit.startAbsoluteSleepTimer(DateTime.now().add(const Duration(hours: 1)));
      ...
      when(() => mockRepository.getSongById(303))
      ...
      when(() => mockRepository.getSongsByIds([303]))
          .thenAnswer((_) async => Right([songC]));
    ...
    test('AudioService stream events update PlayerState', () async {
      ...
      when(() => mockRepository.getSongById(777))
          .thenAnswer((_) async => Right(song));
      when(() => mockRepository.getSongsByIds([777]))
          .thenAnswer((_) async => Right([song]));
      ...
      await Future<void>.delayed(const Duration(milliseconds: 50));
      expect(cubit.state.sleepTimerRemaining, equals(const Duration(minutes: 20)));
