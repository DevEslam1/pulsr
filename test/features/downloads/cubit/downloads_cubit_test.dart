...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockStorageStats.call()).thenAnswer(
      (_) async => const Right(
        StorageStats(
          usedBytes: 1024,
          freeBytes: 10240,
  ...
  group('DownloadsCubit', () {
    ...
    test(
      'clearError resets errorMessage and failure in DownloadsState',
      () async {
        when(() => mockQueue.call(testTask)).thenAnswer(
    ...
    test(
      'DownloadsState copyWith preserves error fields unless explicitly cleared',
      () {
    ...
    test(
      'Move 4: 1000 progress ticks in 1s throttle to <=6 emissions and terminal complete arrives',
      () async {
        final cubit = buildCubit();
        await Future.delayed(const Duration(milliseconds: 50));
        ...
        // Terminal event
        downloadStreamController.add(
          testTask.copyWith(status: DownloadStatus.complete, progress: 1.0),
        );
        ...
        expect(
          cubit.state.tasks[testTask.videoId]?.status,
    ...
    test(
      'Move 3: per-task actions serialize correctly without deadlock',
      () async {
    ...
    test(
      'Move 7: typed failures provide l10nKey and actionable recovery type',
      () {
        ...
        const f2 = InsufficientStorageFailure(
          'Low disk',
          neededBytes: 100,
          availableBytes: 50,
        );
    ...
    test(
      'pauseDownload, resumeDownload, retryDownload, deleteDownload handle failures cleanly',
      () async {
        when(
          () => mockPause.call('err_vid'),
        ).thenAnswer((_) async => const Left(DownloadFailure('Pause fail')));
        when(
          () => mockResume.call('err_vid'),
        ).thenAnswer((_) async => const Left(DownloadFailure('Resume fail')));
        when(
          () => mockRetry.call('err_vid'),
        ).thenAnswer((_) async => const Left(DownloadFailure('Retry fail')));
        when(
          () => mockDelete.call('err_vid'),
        ).thenAnswer((_) async => const Left(DownloadFailure('Delete fail')));
        when(() => mockPrioritize.call('err_vid')).thenAnswer(
          (_) async => const Left(DownloadFailure('Prioritize fail')),
        );
        when(
          () => mockReorder.call(['v1', 'v2']),
        ).thenAnswer((_) async => const Left(DownloadFailure('Reorder fail')));
    ...
    test(
      'Task actions catch unhandled exceptions without throwing or crashing',
      () async {
        when(
          () => mockPause.call('crash_vid'),
        ).thenThrow(Exception('Simulated crash'));
        when(
          () => mockResume.call('crash_vid'),
        ).thenThrow(Exception('Simulated crash'));
        when(
          () => mockRetry.call('crash_vid'),
        ).thenThrow(Exception('Simulated crash'));
        when(
          () => mockDelete.call('crash_vid'),
        ).thenThrow(Exception('Simulated crash'));
        when(
          () => mockPrioritize.call('crash_vid'),
        ).thenThrow(Exception('Simulated crash'));
        when(
          () => mockStorageStats.call(),
        ).thenAnswer((_) async => const Left(StorageFailure('Stats fail')));
    ...
    test(
      '[B1] deleteDownload immediately followed by queueDownload and prioritizeDownload executes sequentially without race or deadlock',
      () async {
