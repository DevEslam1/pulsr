...
void main() {
  ...
  group('Interleaved race scenarios (Completer-driven fakes)', () {
    ...
    test(
      ...
      expect(toasts, isEmpty, reason: 'duplicate start must not raise a toast');
      expect(errors, isEmpty);
    ...
    test('retry during delete: actions serialize, delete wins, no resurrection',
        () async {
      ...
      expect(errors, isEmpty);
      expect(cubit.state.tasks.containsKey('vid_dead'), isFalse,
    ...
    test('network drop -> transient failure -> retry success clears error',
        () async {
      when(() => mockQueue.call(any()))
          .thenAnswer((_) async => const Left(NetworkFailure('Network down')));
      when(() => mockRetry.call(any()))
          .thenAnswer((_) async => const Right(unit));
      ...
      expect(cubit.state.isErrorRetryable, isTrue,
    ...
    test('dual pause/resume on the same task: serialized, final state paused',
        () async {
      ...
      expect(cubit.state.tasks['vid_pr']?.status, DownloadStatus.paused,
          reason: 'the dual pause/resume race must settle on paused');

      await cubit.close();
    });

    test('reorder during completion: both land, no error, no lost task',
        () async {
      final mockReorder = MockReorderDownloadsUseCase();
      final reorderGate = Completer<Either<AppFailure, Unit>>();
      when(() => mockReorder.call(any())).thenAnswer((_) => reorderGate.future);
      when(() => mockObserve.getAll()).thenAnswer((_) async => [
      ...
      final reorderFuture =
          cubit.reorderQueue(['vid_b', 'vid_a']);
      await Future<void>.delayed(const Duration(milliseconds: 20));
      ...
      expect(cubit.state.tasks.containsKey('vid_a'), isTrue,
