...
void main() {
  ...
  group('Downloads Chaos v2 & Concurrency Suite', () {
    test(
      '1000 seeded runs with prune-race assert zero overlapping executions and zero leaks',
      () async {
        const baseSeed = 1337;
        ...
        Future<void> executeTracked(
          ...
          expect(
            current,
            lessThanOrEqualTo(1),
            reason: 'Concurrent execution detected for task $id',
          );
          try {
            await work();
          } finally {
        ...
        when(() => mockQueue.call(any())).thenAnswer((inv) async {
          ...
          await executeTracked(
            task.videoId,
            () async => await Future.delayed(const Duration(microseconds: 50)),
          );
        ...
        for (int run = 0; run < 1000; run++) {
          ...
          try {
            ...
            expect(
              cubit.state.activeCount,
              greaterThanOrEqualTo(0),
              reason: 'Failed activeCount on seed $runSeed',
            );
            expect(
              cubit.state.pausedCount,
              greaterThanOrEqualTo(0),
              reason: 'Failed pausedCount on seed $runSeed',
            );
            expect(
              cubit.state.completedCount,
              greaterThanOrEqualTo(0),
              reason: 'Failed completedCount on seed $runSeed',
            );
            expect(
              cubit.state.taskList.length,
              equals(cubit.state.tasks.length),
              reason: 'Failed list conservation on seed $runSeed',
            );
          } catch (e) {
        ...
        expect(
          ...
          lessThanOrEqualTo(1),
          reason: 'Zero overlapping operations guaranteed across all 1000 runs',
        );
    ...
    test(
      'app kill during download: fresh cubit rehydrates persisted tasks',
      () async {
        final mockObserve = MockObserveDownloadsUseCase();
        final mockQueue = MockQueueDownloadUseCase();
        final mockPause = MockPauseDownloadUseCase();
        ...
        expect(
          cubit.state.tasks.length,
          equals(persisted.length),
        ...
        expect(
          cubit.state.tasks['killed_paused']?.status,
          equals(DownloadStatus.paused),
        );
        expect(
          cubit.state.tasks['killed_done']?.status,
    ...
    test(
      ...
      () async {
        ...
        expect(
          status.values.where((s) => s == DownloadStatus.downloading).length,
          0,
        );
        ...
        expect(
          status.values.where((s) => s == DownloadStatus.failed).length,
          0,
        );
        expect(
          status.values.where((s) => s == DownloadStatus.complete).length,
          greaterThan(0),
        );
      },
    );
