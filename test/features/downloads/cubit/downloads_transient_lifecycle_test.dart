...
void main() {
  ...
  group('Transient Error Lifecycle & Terminal Flush Tests', () {
    test('Transient error is cleared upon explicit clearError() call', () async {
      ...
      when(() => mockQueue.call(task)).thenAnswer((_) async => const Left(NetworkFailure('Network down')));
    ...
    test('Terminal FAILED event flushes within bounded <= 350ms window', () async {
      final mockQueue = MockQueueDownloadUseCase();
      final mockPause = MockPauseDownloadUseCase();
      final mockResume = MockResumeDownloadUseCase();
      final mockRetry = MockRetryDownloadUseCase();
      ...
      // Await arrival
      while (cubit.state.byId('task_flush')?.status != DownloadStatus.failed && stopwatch.elapsedMilliseconds < 1000) {
        await Future.delayed(const Duration(milliseconds: 10));
      }
      stopwatch.stop();

      expect(cubit.state.byId('task_flush')?.status, equals(DownloadStatus.failed));
      expect(stopwatch.elapsedMilliseconds, lessThanOrEqualTo(350), reason: 'Terminal failed event must flush within 350ms');
