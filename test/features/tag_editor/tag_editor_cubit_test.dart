...
void main() {
        ...
        switch (call.method) {
          case 'readTags':
            ...
            if (path != null && path.contains('unsupported')) {
              throw PlatformException(code: 'UNSUPPORTED_FORMAT', message: 'Format not supported');
            }
            return {
              'title': 'Read Title',
              'artist': 'Read Artist',
          ...
          case 'writeTags':
            ...
            if (path != null && path.contains('fail_write')) {
              throw PlatformException(code: 'WRITE_ERROR', message: 'Write failed');
            }
            return true;
          default:
            return null;
  ...
  group('[T1] TagEditorCubit Comprehensive Test Suite', () {
    ...
    test('Field editing and validation (Arabic RTL, Unicode emojis, long strings)', () async {
      final cubit = TagEditorCubit(
        song: testSong,
        scannerService: mockScanner,
        metadataSearchService: mockMetadataSearch,
    ...
    test('Save success invokes writeTags and triggers scanner refresh', () async {
      when(() => mockScanner.rescanSingleFile(any())).thenAnswer((_) async {});
      ...
      final writeCall = channelCalls.firstWhere((c) => c.method == 'writeTags');
    ...
    test('Save failure preserves state and sets error status without crashing', () async {
      when(() => mockScanner.rescanSingleFile(any())).thenAnswer((_) async {});

      final failingSong = testSong.copyWith(path: '/storage/emulated/0/Music/fail_write.mp3');
      final cubit = TagEditorCubit(
        song: failingSong,
        scannerService: mockScanner,
        metadataSearchService: mockMetadataSearch,
      ...
      expect(cubit.state.title, equals('New Title')); // Preserved edits for retry
      cubit.close();
    });

    test('Batch mode applies common fields across all selected tracks', () async {
      final batchSong1 = testSong.copyWith(id: 1, path: '/path/1.mp3', artist: 'Artist A');
      final batchSong2 = testSong.copyWith(id: 2, path: '/path/2.mp3', artist: 'Artist B');
      ...
      final writeCalls = channelCalls.where((c) => c.method == 'writeTags').toList();
    ...
    test('Tag format matrix (MP3 ID3v2.3, ID3v2.4, FLAC Vorbis, MP4 AAC)', () async {
      final formats = [
        '/music/track.mp3',
        '/music/track.flac',
        '/music/track.m4a',
