...
void main() {
  group('YtmAccountService.looksLikeSignedInCookies', () {
    ...
    test('accepts secure-variant combinations', () {
      const raw = '__Secure-3PAPISID=tok; __Secure-1PSID=tok2; LOGIN_INFO=x';
      expect(YtmAccountService.looksLikeSignedInCookies(raw), isTrue);
    });
  ...
  group('YtmWebLoginSheet URL classification', () {
    ...
    test('matches CookieMismatch and block error page URLs', () {
      expect(
        isCookieMismatchUrl.hasMatch(
            'https://accounts.google.com/CookieMismatch?continue=...'),
        isTrue,
      );
      expect(
        isCookieMismatchUrl
            .hasMatch('https://music.youtube.com/sorry/index?continue=...'),
        isTrue,
      );
    });

    test(
        'matches Google Sign-In and consent URLs without classifying them as mismatch',
        () {
      expect(
        isAuthInProgressUrl.hasMatch(
      ...
      expect(
        isAuthInProgressUrl
            .hasMatch('https://accounts.google.com/v3/signin/identifier'),
        isTrue,
      );
      expect(
        isCookieMismatchUrl
    ...
    test('does not match normal YouTube Music browsing URLs', () {
      expect(
          isCookieMismatchUrl.hasMatch('https://music.youtube.com/'), isFalse);
      expect(
          isAuthInProgressUrl.hasMatch('https://music.youtube.com/'), isFalse);
      expect(isCookieMismatchUrl.hasMatch('https://music.youtube.com/library'),
          isFalse);
      expect(isCookieMismatchUrl.hasMatch('https://music.youtube.com/explore'),
          isFalse);
      expect(
          isCookieMismatchUrl
  ...
  group('YtmAccountService.buildAuthorizationHeader', () {
    test('builds SAPISIDHASH when SAPISID is present', () {
      final header = YtmAccountService.buildAuthorizationHeader(
          'SAPISID=test_sapisid_123; __Secure-3PSID=psid');
    ...
    test('builds SAPISID3PHASH when only __Secure-3PAPISID is present', () {
      final header = YtmAccountService.buildAuthorizationHeader(
          '__Secure-3PAPISID=test_3papisid_456; __Secure-3PSID=psid');
      expect(header, isNotNull);
    ...
    test('builds SAPISID1PHASH when only __Secure-1PAPISID is present', () {
      final header = YtmAccountService.buildAuthorizationHeader(
          '__Secure-1PAPISID=test_1papisid_789; __Secure-1PSID=psid');
      expect(header, isNotNull);
