...
class MissingArtworkService {
  ...
  Future<String?> fetchArtworkForAlbum(String albumTitle, String artistName) async {
    try {
      final query = '$albumTitle $artistName'.trim();
      final uri = Uri.parse('https://itunes.apple.com/search?term=${Uri.encodeComponent(query)}&entity=album&limit=1');
      ...
      ErrorLogger.log('Failed to fetch artwork for $albumTitle', error: e, stackTrace: st, category: 'MissingArtworkService');
    }
    return null;
  }
